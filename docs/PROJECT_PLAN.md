...
ファイル型、ストリーミング型の統合について、本来の設計思想（キューベースの取り出し、VADチャンク分割、文脈管理と結合、Whisper推論のリアルタイム進行、前処理・後処理の連携）を満たすように詳細設計をやり直す。

### Phase 14: モジュール機能分割リファクタリング
「1モジュール = 1機能」の原則を徹底し、肥大化したモジュールを責務単位で分割する。実装前に現状を精査して分割方針を策定してから着手する。

### Phase 15: 耐障害性およびメトリクス通知の実装
CUDA OOM 等のランタイムエラーからの安全なリカバリ処理と、E2E レイテンシ・バッファ残量等の稼働メトリクス計測・通知機能を実装する。

### Phase 16: ライブラリ標準インターフェースとアーキテクチャの準拠
要件仕様書で定義されたインターフェース設計（`RecognizedSegment`・`reset()`・Factory DI）に対する現状差分を解消する。

### Phase 17: GPU/デバイス不要テスト基盤の整備
