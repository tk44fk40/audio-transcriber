...
- [ ] 14.1.3 オーケストレーターとプロデューサーにおける堅牢化課題（課題A、課題B）の実装修正
  - `AudioChunkQueue` の `maxsize` を、1チャンク時間長、サンプリングレート、および総蓄積目標秒数から動的に算出・設定するロジックの実装（課題Aの修正）
  - `FileAudioProducer` へ `denoised_audio_path` プロパティを実装し、一時ファイルのクリーンアップ削除権限を上位（`PipelineSupervisor` など）へ引き上げる（課題Bの修正）
  - 課題A・課題Bの修正に対する単体テスト・結合テストの追加・更新
- [ ] 14.1.4 `file_remuxer.py` の作成
