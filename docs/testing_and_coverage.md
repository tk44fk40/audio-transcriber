...
### (2026-08-16: Phase 14 セルフチェック点検)

| 対象モジュール | 未カバーだった行 | 当初の理由 | 評価判定 | 対策内容 |
|---|---|---|---|---|
| `audio_producers.py` | 284, 304-305, 313-314, 380-388, 393, 415, 442-448 | ライフサイクル停止、FFmpeg例外、極限防衛ロジック、センチネル投入タイムアウト | **テスト可能 (改善)** | モックプロセス・モックキューおよび `BaseException` 注入を用いてこれらすべての防衛コードと例外ログを完全カバーし、カバレッジ100%を達成 |
| `pipeline_supervisor.py` | 236-237 | `run_file` 処理中の外部 `stop` 操作による途中終了 | **検証妥当** | ファイルバッチ実行中に非同期にライフサイクルが停止された場合の安全ガード。 |
| `pipeline_supervisor.py` | 244-251 | `run_file` 実行中のデコード/中継例外発生時ハンドリング | **検証妥当** | 例外が起きた際にコールバックへ通知し、リソース（producer）を安全にクリーンアップして再送出する例外堅牢性。 |
