...
def test_scenario_1_gaming_commentary_multitrack(tmp_path: Path) -> None:
    ...
    with (
        ...
        result = run_pipeline(
            ...
            output_dir=out_dir,
            mic_track=2,
            initial_prompt="Apex, ヴァルキリー, クレーバー",
            denoise=True,
            transcribe=True,
            remux=True,
        )
...
def test_scenario_2_technical_keynote_transcribe_only(tmp_path: Path) -> None:
    ...
    with patch(
        ...
        result = run_pipeline(
            ...
            output_dir=out_dir,
            model_size="large-v3",
            initial_prompt="DeepFilterNet, faster-whisper, 第VIII章",
            denoise=False,
            transcribe=True,
        )
    ...
    assert "第VIII章" in kwargs["initial_prompt"]


def test_scenario_3_conversational_turn_taking_vad(tmp_path: Path) -> None:
...
def test_scenario_4_high_noise_podcast_denoise_and_transcribe(tmp_path: Path) -> None:
...
""",
    ...
    assert kwargs["output_dir"] == tmp_path / "davinci_out"
    assert kwargs["initial_prompt"] == "DaVinci Resolve 編集プロジェクト"
